...
# silent
x <- text_replace(x, "@" = ">>>", verbose = 0)

# detailed per-shape output
x <- read_pptx(file_in)
x <- text_replace(x, "@" = ">>>", verbose = 2)
```
...
Patterns are matched as fixed strings (no regex). This means shorter patterns can match
inside longer ones. For example, `"[2]"` also matches inside `"[[2]]"` since it is a
substring.

To avoid unintended substring matches, replace longer patterns first:
